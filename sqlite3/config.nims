...
let sqlite_CFLAGS = [
  ...
  "-DSQLITE_THREADSAFE=0",              # Single Threaded, since my app doesn't use threads
...
task static_sqlite, "statically link executable with sqlite3":
  if not fileExists(sqlite_ofile):
    build_sqliteTask()
  let sqlite_LFLAGS = fmt"{sqlite_ofile}"   # Add -lm -pthread if compiling with threads on
  --dynlibOverride: sqlite3
  switch("passL", sqlite_LFLAGS)
  setCommand "c"

task clean, "clean":
  exec fmt"rm -f {sqlite_ofile}"
